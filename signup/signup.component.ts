...
console.log(d1,d2,d3,d4,d5);
  if(d1.value==' '||d1.value=='' || d2.value==''||d2.value==' ' || d3.value==''||d3.value==' ' ||d4.value==''||d4.value==' '|| d5.value==' '||d5.value==''){
    ...
    alert("Password doesn't matched");
          }else{if(!this.emailTest.test(d2.value))
            {
    ...
    // this.arr[0].name=d1.value;

    this.route.navigate(['/login']);//this.arr[0].name]);
  //alerttr("please login now ",'Thank you for Sign up ',{position:'top-center'});
...
this.service.reg(d1.value,d2.value,d3.value,d4.value,d5.value).subscribe((res:any)=>{ 
  ...
  // "E11000 duplicate key error collection: selling.sellings index: contact_1 dup key: { : "8619074467" }"
  if(err.status === 403)
  {
   alert(err.error.msg+" "+"please fill all the credentials again properly",);
console.log(err.token);
// this.route.navigate[('/signup')];
    // let duplicate = err.error.errmsg.split(":")[2].split(" ")[1];
